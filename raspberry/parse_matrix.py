...
def normalize(matrix, min, max):
    return (matrix-min)/float(max-min)
        ...
        for j in range(x.shape[1]):
            ...
            for o_i in range(operator.shape[0]):
                for o_j in range(operator.shape[1]):
                    if i+o_i-1<0 or j+o_j-1<0 or i+o_i-1>=x.shape[0] or j+o_j-1>=x.shape[1]:
                        result_ij += operator[o_i][o_j]*x[i][j]
                    else:
                        result_ij += operator[o_i][o_j]*x[i+o_i-1][j+o_j-1]
            result[i][j] = result_ij
    
    return result

def get_mean_Laplace(matrix):
    laplace_operator = np.array([[1.0, 1.0, 1.0],
                        [1.0, -8.0, 1.0],
                        [1.0, 1.0, 1.0]])
    
    laplace_result = conv2d(matrix, laplace_operator)
    return np.abs(laplace_result.mean())
...
def get_matrix_feature(matrix_normalized):
    laplace_mean = get_mean_Laplace(matrix_normalized)
    all_laplace_min = 0
    all_laplace_max = 2e-16 #3.9968e-16
...
# 矩阵是否发生变化
def matrix_change(current, last, threshold):
    diff = np.abs(last-current).sum()
    if diff>threshold:
        return True
    else:
        return False
