...
# 给音符加上音高，比如[C, D],[2,3] -> [C2,D2,C3,D3]
...
#输出和弦进行，每个bar是一组和弦，每个bar可能是四个四分音符，两个二分音符或一个一分音符
def create_chord_progression(chord_notes, note_octave, Valence, Valence_range):
    chord_progression = []
...
# last_x是candidates中的一员，输出与candidates同样长度的概率值，其中距离last_x位置越近的概率越大（0与len-1距离最远而不是近）
...
# 输出旋律，每个音符的音程随机（选择概率与上个音程长度有关，里上一音程越近概率越大），音高由matrix_list中此刻的值决定，有20%的概率是空拍
...
def get_basic_setting(Valence, Valence_range):
    ...
    return progression, note_octave, arpeggios
...
def generate_music_from_Valence(Valence, Valence_range, matrix_list):
    ...
    # 和弦进行
    chord_progression, chord_indexes = create_chord_progression(chord_notes, note_octave, Valence, Valence_range) 
...
if __name__ == '__main__':
    ...
    chord_progression, melody, settings = generate_music_from_Valence(Valence, Valence_range, np.zeros(5*4))#np.random.rand(5*4))
